...
const S = {
    ...
    metricHist: {},
    loadHist: {}
}; // hist latency; metricHist: {key:{cpu:[],mem:[],hdd:[]}}; loadHist: {key:{l1:[],l5:[],l15:[]}}
    ...
    try {
        ...
        S.servers.forEach((s, idx) => {
            ...
            if (s.online4 || s.online6) {
                ...
                const MAXM = 120;
                ['cpu', 'mem', 'hdd'].forEach(k => { if (MH[k].length > MAXM) MH[k].splice(0, MH[k].length - MAXM); });
            }
...
function render() {
    ...
    els.last().textContent = humanAgo(S.updated);
    renderServers();
    renderMonitors();
    renderSSL();
    renderServersCards();
...
function renderServers() {
    ...
    S.servers.forEach((s, idx) => {
        ...
        const HEAVY_THRESHOLD = 500 * 1000 * 1000 * 1000;
  ...
  <td>${pingBuckets}</td>
    ...
    tbody.innerHTML = html || '<tr><td colspan="13" class="muted">无服务器数据</td></tr>';
    tbody.querySelectorAll('tr.row-server').forEach(tr => {
        tr.addEventListener('click', () => {
            if (tr.getAttribute('data-online') !== '1') return;
...
function gaugeHTML(type, val) {
    ...
    const warn = pct >= 50 ? (pct >= 90 ? ' data-danger' : ' data-warn') : '';
    return `<div class="gauge-half" data-type="${type}"${warn} style="--p:${p}">
      <svg viewBox="0 0 100 55"><path class="track" d="M10 50 A40 40 0 0 1 90 50"/><path class="arc" d="M10 50 A40 40 0 0 1 90 50"/></svg>
...
function renderServersCards() {
    ...
    wrap.querySelectorAll('.card').forEach(card => {
        ...
        card.addEventListener('click', () => {
            if (card.getAttribute('data-online') !== '1') return;
            openDetail(idx);
        });
    });
}
...
function renderMonitorsCards() {
  ...
  <div><span class="key">监测内容</span><span class="mon-items">${row || '-'}</span></div>
    ...
    wrap.innerHTML = html || '<div class="muted" style="text-align:center;padding:1rem;">无服务监测数据</div>';
}
...
function renderSSL() {
    ...
    S.ssl.forEach(c => {
      ...
      <td class="mono">${c.expire_days || 0} 天</td>
      <td class="mono" style="font-size:11px;">${dt}</td>
    </tr>`;
    });
    tbody.innerHTML = html || '<tr><td colspan="6" class="muted">无证书监控数据</td></tr>';
}
...
function renderSSLCards() {
    ...
    S.ssl.forEach(c => {
      ...
      <div class="kvlist" style="grid-template-columns:repeat(2,minmax(0,1fr));">
        ...
        <div><span class="key">剩余</span><span>${c.expire_days || 0} 天</span></div>
        <div><span class="key">到期</span><span style="font-size:10px;">${dt}</span></div>
      </div>
    </div>`;
    });
    wrap.innerHTML = html || '<div class="muted" style="text-align:center;padding:1rem;">无证书监控数据</div>';
}

function updateTime() {
    if (S.updated) els.last().textContent = humanAgo(S.updated);
}
...
function drawLoadChart(key) {
    ...
    ctx.clearRect(0, 0, W, H);
    if (l1.length < 2) {
        ctx.fillStyle = 'var(--text-dim)'; ctx.font = '12px system-ui';
    ...
    let range = max - min;
    if (range < 0.5) { max = 0.5; range = 0.5; }
    ...
    series.forEach(s => {
        ...
        if (s.fill) {
            ...
            grd.addColorStop(0, 'rgba(139,92,246,0.25)'); grd.addColorStop(1, 'rgba(139,92,246,0)');
...
function updateDetailMetrics(key) {
    const s = findServerByKey(key); if (!s) return; if (!(s.online4 || s.online6)) return;
    const procLine = `${num(s.tcp_count)} / ${num(s.udp_count)} / ${num(s.process_count)} / ${num(s.thread_count)}`;
